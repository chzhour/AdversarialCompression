...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.operation = nn.Sequential( nn.Linear(10 + 1, 400), #10+1
                                        nn.ReLU(),
                                        nn.Dropout(),
...
def train_multi_students(args, teacher_model, student_models, discriminator, device, train_loader, 
                            optimizer_student, optimizer_discriminator, epoch):
    teacher_model.train()
    ...
    for batch_idx, (data, c_target) in enumerate(train_loader):
        ...
        classifier_output = []
        with torch.no_grad():
            classifier_output.append( teacher_model(data)[0] )
        for m in student_models:
            classifier_output.append( m(data)[0] )
        classifier_output = torch.cat(classifier_output, dim=0)
        c_target = c_target.float().unsqueeze_(dim=1)*1.0 # int64 to float32, (size) to (size, 1) for cat
        c_target = c_target.repeat(len(student_models)+1, 1) #duplicated tensor
        classifier_output = torch.cat((classifier_output, c_target), dim=1)

        d_target = [torch.zeros(data.size()[0], dtype=torch.int64)]
        for i in range(len(student_models)):
            d_target.append( torch.ones(data.size()[0], dtype=torch.int64) * (i+1) )
        d_target = torch.cat(d_target, dim=0).to(device)
...
def test_multi_models(args, models, device, test_loader):
    for model in models:
        model.eval()
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            for idx, model in enumerate(models):
                ...
                test_loss[idx] += F.nll_loss(torch.log(prob), target, size_average=False).item() # sum up batch loss
                pred = output.max(1, keepdim=True)[1] # get the index of the max logits before softmax
                correct[idx] += pred.eq(target.view_as(pred)).sum().item()
            pred = cumulative_prob.max(1, keepdim=True)[1]
            correct_merged += pred.eq(target.view_as(pred)).sum().item()

    for idx in range(len(models)):
        print('Student {}\tTest Average Loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)'.format(
            idx+1, test_loss[idx]/len(test_loader.dataset), correct[idx], len(test_loader.dataset),
            100. * correct[idx] / len(test_loader.dataset)
            ))

    acy = 1.0 * correct_merged / len(test_loader.dataset)
    print('\nAccuracy of Merged Students: {}/{} ({:.2f}%)\n'.format(
        correct_merged, len(test_loader.dataset), 100. * acy))
    return acy
...
def main():
    ...
    for epoch in range(1, 2):
        ...
        teacher_acy = test(args, teacher_model, device, test_loader)

    ##################################################################
    student_models = []
    optimizer_student = [] #list of parameters, will be changed to optimizer
    for i in range(3):
        student_models.append( Net().to(device) )
        student_models[-1].fc2.register_backward_hook(reverse_grad_hook)
        optimizer_student += list(student_models[-1].parameters())
    discriminator = Discriminator().to(device)
    #optimizer = optim.SGD(list(student_model.parameters()) + list(discriminator.parameters()), lr=args.lr, momentum=args.momentum)
    optimizer_student = optim.SGD(optimizer_student, lr=args.lr, momentum=args.momentum)
    optimizer_discriminator = optim.SGD(discriminator.parameters(), lr=args.lr, momentum=args.momentum)
    print(student_models[0])
    print(discriminator)
    #print(optimizer)
    ...
    for epoch in range(1, args.epochs + 1):
        train_multi_students(args, teacher_model, student_models, discriminator, device, train_loader, 
                                optimizer_student, optimizer_discriminator, epoch)
        if epoch == 1:
            assert(teacher_acy == test(args, teacher_model, device, test_loader))
        else:
            print('Teacher Test Accuracy: {:.2f}%\n'.format(100.0 * teacher_acy))
        acy = test_multi_models(args, student_models, device, test_loader)
        
        if(best_student_acy < acy):
            best_student_acy = acy
        print('Best Test Accuracy of Merged Students: {:.2f}%\n'.format(100.0 * best_student_acy))
